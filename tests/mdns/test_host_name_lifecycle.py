...
def _write_svc(
    server, filename="smb.conf", instance_name="%h",
...
class TestSighupDoesNotRename:
    ...
    def test_swapped_domain_never_leaks_into_conflict_rename(
        self, server_with_service,
    ):
        """domain-name is startup state exactly like host-name: a
        SIGHUP-swapped domain edit that ``_reload`` refused must not
        half-apply later, when a §9 conflict rename rebuilds the
        FQDN."""
        server, loop = server_with_service
        server.apply_config(DaemonConfig(
            server=ServerConfig(host_name="nas", domain_name="lan"),
            service_dir=server._config.service_dir,
            rundir=server._config.rundir,
        ))
...
class TestRenameConvergence:
    ...
    def test_second_conflict_during_rebuild_renames_again(
        ...
        try:
            ...
            assert server._rehoming is False
            # The failed nas-2 attempt spent restart budget; the
            # nas-3 convergence must hand it back.
            assert server._rename_restarts == 0
...
class TestRenameReloadSerialization:
    """A §9 rename and a SIGHUP reload both rebuild registry state
    across awaits; ``_rebuild_lock`` keeps each transition whole.

    Without it, a host conflict landing inside the delta reload's
    probe of an added service lets ``_reregister_all`` replace
    ``_entry_groups`` / ``_service_groups`` mid-loop — the delta
    then appends stale-keyed groups next to the rebuilt ones and
    the same service ends up registered twice under two names.
    """

    def test_delta_reload_and_rename_serialize(
        self, tmp_path, make_server, started_state,
    ):
        loop = asyncio.new_event_loop()
        server = make_server(tmp_path, hostname="nas")
        _write_svc(server, filename="smb.conf")
        state = started_state(loop, server._config, InterfaceInfo(
            name="lan0", index=LAN_INDEX, addrs_v4=[LAN_V4],
        ))
        server._interfaces = {LAN_INDEX: state}
        try:
            ...
            # Real probing from here on: each added service takes
            # ~1s to probe.  Two additions matter — the conflict
            # lands inside the first one's probe, and the second
            # one's append is what would go stale: its key was
            # computed under the old name before the rename replaced
            # the registry.
            state.prober = Prober(
                lambda *a, **kw: None, server._on_conflict,
            )
            _write_svc(
                server, filename="nfs.conf",
                service_type="_nfs._tcp", port=2049,
            )
            _write_svc(
                server, filename="http.conf",
                service_type="_http._tcp", port=80,
            )

            async def race() -> None:
                reload_task = asyncio.create_task(
                    server._service_delta_reload(),
                )
                # Land the conflict inside the delta's probe.
                ...
                await asyncio.gather(
                    reload_task,
                    server._resolve_conflict([]),
                )

            loop.run_until_complete(race())

            assert server._fqdn == "nas-2.local"
            # Every key was computed under one name: no phantom
            # old-name keys, no duplicate groups for one service.
            assert len(server._service_groups) == 3
            assert {
                k.instance_name for k in server._service_groups
            } == {"nas-2"}
            assert len(server._entry_groups) == (
                len(server._service_groups) + len(server._host_groups)
            )
            assert _srv_targets(server) == {"nas-2.local"}
            assert _host_names(server) == {"nas-2.local"}
        finally:
...
class TestRenameRestartCap:
    ...
    def test_rename_gives_up_after_max_restarts(
        self, tmp_path, make_server, started_state,
    ):
        loop = asyncio.new_event_loop()
        server = make_server(tmp_path, hostname="nas")
        ...
        try:
            ...
            # A zero-latency peer that owns *every* name we might
            # try: each probe question is answered straight back
            # into the prober with greater rdata (203.0.113.200 >
            # 192.0.2.10), so every §8.2 tiebreak is lost and no
            # rename can converge.
            def defend_everything(msg: MDNSMessage) -> None:
                answers = [
                    MDNSRecord(
                        key=MDNSRecordKey(q.name, QType.A),
                        ttl=120,
                        data=ARecordData(IPv4Address("203.0.113.200")),
                        cache_flush=True,
                    )
                ...
                if answers:
                    state.prober.handle_incoming(
                        MDNSMessage.build_response(answers),
                        ("203.0.113.200", MDNS_PORT),
                    )
            ...
            async def scenario() -> None:
                ...
                await asyncio.wait_for(
                    server._rename_host_after_conflict(), timeout=30,
                )
                # Drain the conflict task the losing probe queued —
                # the budget guard must turn it into a no-op instead
                # of a fresh rename loop.
                await asyncio.gather(
            ...
            # Gave up: the host group stays withdrawn (COLLISION,
            # absent from the registry) instead of looping.
            assert server._rehoming is False
            assert server._rename_restarts == MAX_PROBE_RESTARTS
            assert all(
                g.state == EntryGroupState.COLLISION
                for g in server._host_groups
            )
            assert server._registry.lookup(
                server._fqdn, QType.A, LAN_INDEX,
            ) == []
            # Exactly one rename was attempted before the cap — and
            # the queued conflict task didn't move it further.
            assert server._fqdn == "nas-2.local"

            # A later conflict-driven call is also a no-op until a
            # full rebuild or restart resets the budget.
