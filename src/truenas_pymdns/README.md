...
| [Apple mDNSResponder](https://github.com/apple-oss-distributions/mDNSResponder) tag [`mDNSResponder-2881.0.25`](https://github.com/apple-oss-distributions/mDNSResponder/releases/tag/mDNSResponder-2881.0.25) (`mDNSCore/`, `mDNSPosix/`) | Apache-2.0 | Timing constant tuning (`mDNS.c`: goodbye count, host record TTL, `MAX_PROBE_RESTARTS`), simultaneous-probe tiebreak + 1 s defer (`ResolveSimultaneousProbe`), stale-packet tolerance (`kMaxAllowedMCastProbingConflicts`), §9 same-name re-probe on established conflict (`mDNSCoreReceiveResponse`), shared-record goodbye on conflict rename (`mDNS_Deregister_internal`), unique-without-probing for reverse address mapping PTRs and their discard-on-conflict (`kDNSRecordTypeKnownUnique` in `AdvertiseInterface` and `mDNSCoreReceiveResponse`), netlink link-state monitor + flap-delay/reduced-announce semantics (`OpenIfNotifySocket`, `mDNS_RegisterInterface`) |
