...
class MDNSServer(ConfigDaemon):
    ...
    def __init__(self, config: DaemonConfig) -> None:
        ...
        # Flap-detection state per ifindex (mDNS_RegisterInterface):
    ...
    async def _start(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        # RTMGRP_LINK netlink subscription (``OpenIfNotifySocket`` in
    ...
    def _register_host_addresses(self) -> None:
        ...
        avahi-core/iface.c) — as does mDNSResponder, whose
    ...
    async def _resolve_conflict(self, records: list[MDNSRecord]) -> None:
        ...
        for group in self._entry_groups:
            ...
            # (mDNSCore/mDNS.c), which only schedules TTL=0
    ...
    async def _rename_host_after_conflict(self) -> None:
        ...
        it from ``_config``, so it survives every SIGHUP.  Neither
        reference implementation persists it across a restart and
        neither do we — Avahi likewise re-probes from the configured
        name on startup and renames again if the conflict is still
        there.
        ...
        try:
            while True:
                old_fqdn = self._fqdn
                self._hostname = generate_alternative_name(self._hostname)
                self._fqdn = (
                    f"{self._hostname}.{self._config.server.domain_name}"
                )
                logger.warning(
                    "Host name conflict: %s -> %s", old_fqdn, self._fqdn,
                )
    ...
    def _check_established_conflicts(
        ...
        branch in ``mDNSCoreReceiveResponse`` (mDNSCore/mDNS.c):
    ...
    async def _on_link_up(self, ifindex: int) -> None:
        ...
        (``mDNS_RegisterInterface``).  The 0.5s normal probe delay
        ...
        announcement count to one packet."*, ``mDNS_RegisterInterface``).
...
def _obsolete_shared_records(
    ...
    ``mDNS_Dereg_conflict`` (mDNSCore/mDNS.c), which only
